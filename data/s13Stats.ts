...
export const S13_STATS_CSV_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vSXs54PAbDnp_PgxYIigdldr72bC0Pm3BSYzm5f7acohgjTv342vA0n87GsonKu82P2A3mg6x45v7iV/pub?gid=0&single=true&output=csv';
...
export const mapRow = (raw: Record<string, any>): S13Row => {
...
export const fetchS13Stats = async (opts?: { signal?: AbortSignal }): Promise<S13Row[]> => {
  try {
    const response = await fetch(S13_STATS_CSV_URL, { signal: opts?.signal });
    if (!response.ok) throw new Error('Failed to fetch S13 stats');
    ...
    return new Promise((resolve, reject) => {
      ...
      parseFn(csvData, {
        ...
        complete: (results: any) => {
          ...
          rows.sort((a: S13Row, b: S13Row) => b.pts - a.pts);
          resolve(rows);
        },
        error: (error: any) => {
          reject(error);
    ...
    console.error('S13 Stats Fetch Error:', err);
    throw err;
  }
};
