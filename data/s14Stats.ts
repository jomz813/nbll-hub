...
export const S14_STATS_CSV_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vRJOo3GZSlEW7E4Mqmn7-ktNP7zQKM4djiwi0_4ptjmZKn2imY7FihYWln4kV6_MGAFi2asPuCx77F9/pub?gid=0&single=true&output=csv';
...
export const mapRow = (raw: Record<string, any>): S14Row => {
...
export const fetchS14Stats = async (opts?: { signal?: AbortSignal }): Promise<S14Row[]> => {
  try {
    const response = await fetch(S14_STATS_CSV_URL, { signal: opts?.signal });
    if (!response.ok) throw new Error('Failed to fetch S14 stats');
    ...
    return new Promise((resolve, reject) => {
      ...
      parseFn(csvData, {
        ...
        complete: (results: any) => {
          ...
          rows.sort((a: S14Row, b: S14Row) => b.pts - a.pts);
          resolve(rows);
        },
        error: (error: any) => {
          reject(error);
    ...
    console.error('S14 Stats Fetch Error:', err);
    throw err;
  }
};
