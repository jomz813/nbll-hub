            ...
            <div className="w-full">
              ...
              <div className="divide-y divide-zinc-100 dark:divide-zinc-800/50">
                {conf.data.map((row, idx) => {
                  ...
                  const playoffClass = isPlayoffPosition 
                      ? `${accentBgSoft} shadow-[inset_4px_0_0_currentColor] text-inherit` 
                      : '';
                  
                  const stripStyle = isPlayoffPosition ? { boxShadow: `inset 4px 0 0 ${settings.rahBizzyTheme ? '#3B82F6' : '#D60A07'}` } : {};

                  return (
                    <React.Fragment key={idx}>
                      <div 
                        className={`grid grid-cols-[2.5rem_1fr_2.5rem_2.5rem_3.5rem] md:grid-cols-[3rem_1fr_4rem_4rem_5rem_4rem] px-4 md:px-6 ${paddingY} items-center hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors ${playoffClass}`}
                        style={stripStyle}
                      >
