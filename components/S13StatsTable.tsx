...
type SortKey = keyof S13Row;

interface S13StatsTableProps {
  isEmbedded?: boolean;
  season?: string;
...
const getColumnMaxes = (rows: S13Row[], keys: (keyof S13Row)[]) => {
  const maxes: Partial<Record<keyof S13Row, number>> = {};
  keys.forEach(key => {
    const values = rows
...
const S13StatsTable: React.FC<S13StatsTableProps> = ({ isEmbedded = false, season, onSeasonChange, searchQuery = '', externalSortKey, showStat = 'ALL' }) => {
  const { settings, getThemeColors } = useSettings();
  const colors = getThemeColors();
  const accentBg = colors.bg;
  const accentText = colors.text;

  const [data, setData] = useState<S13Row[]>([]);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        setLoading(true);
        setError(null);
        const stats = await fetchS13Stats({ signal: controller.signal });
        if (alive) {
          setData(stats);
        }
      } catch (err: any) {
        if (alive) {
          ...
          setError(err.message || 'Failed to load S13 stats');
        }
      } finally {
        if (alive) {
          setLoading(false);
  ...
  const columnMaxes = useMemo(() => {
    const numericKeys: (keyof S13Row)[] = ['pts', 'ast', 'reb', 'stl', 'gp', 'ppg', 'apg', 'rpg', 'spg', 'eff'];
  ...
  const HighlightedCell = ({ value, colKey, isBold = false, isFormat = false }: { value: number, colKey: keyof S13Row, isBold?: boolean, isFormat?: boolean }) => {
    ...
    <div className={containerClasses}>
              ...
              {filteredData.length > 0 ? (
                ...
                <div className="py-24 text-center">
                  <span className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-400 dark:text-zinc-600">
                    {searchQuery ? `No players found matching "${searchQuery}"` : "No S13 stats yet."}
                  </span>
                </div>
              )}
            </div>
      ...
      <div className="md:hidden space-y-2 px-2">
        {filteredData.length > 0 ? (
          filteredData.map((row, idx) => {
            if (showStat !== 'ALL') {
              const val = row[showStat as keyof S13Row];
      ...
      {!isEmbedded && (
        <div className="flex flex-col md:flex-row items-center justify-center gap-6 opacity-50 px-4 pt-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500">Season 13 Registry</span>
          </div>
        </div>
      )}
    </div>
