...
import { fetchS11Stats, S11Row } from '../data/s11Stats';
import { useSettings } from '../context/SettingsContext';

type SortKey = keyof S11Row;

interface S11StatsTableProps {
  isEmbedded?: boolean;
  season?: string;
...
const getColumnMaxes = (rows: S11Row[], keys: (keyof S11Row)[]) => {
  const maxes: Partial<Record<keyof S11Row, number>> = {};
  keys.forEach(key => {
    const values = rows
...
const S11StatsTable: React.FC<S11StatsTableProps> = ({ isEmbedded = false, season, onSeasonChange, searchQuery = '', externalSortKey, showStat = 'ALL' }) => {
  const { settings, getThemeColors } = useSettings();
  const colors = getThemeColors();
  const accentBg = colors.bg;
  const accentText = colors.text;

  const [data, setData] = useState<S11Row[]>([]);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        setLoading(true);
        setError(null);
        const stats = await fetchS11Stats({ signal: controller.signal });
        if (alive) {
          setData(stats);
        }
      } catch (err: any) {
        if (alive) {
          ...
          setError(err.message || 'Failed to load S11 stats');
        }
      } finally {
        if (alive) {
          setLoading(false);
  ...
  const columnMaxes = useMemo(() => {
    const numericKeys: (keyof S11Row)[] = ['pts', 'ast', 'reb', 'stl', 'gp', 'ppg', 'apg', 'rpg', 'spg', 'eff'];
  ...
  const HighlightedCell = ({ value, colKey, isBold = false, isFormat = false }: { value: number, colKey: keyof S11Row, isBold?: boolean, isFormat?: boolean }) => {
    ...
    <div className={containerClasses}>
      ...
      <div className="md:hidden space-y-2 px-2">
        {filteredData.length > 0 ? (
          filteredData.map((row, idx) => {
            if (showStat !== 'ALL') {
              const val = row[showStat as keyof S11Row];
      ...
      {!isEmbedded && (
        <div className="flex flex-col md:flex-row items-center justify-center gap-6 opacity-50 px-4 pt-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500">Season 11 Registry</span>
          </div>
        </div>
      )}
    </div>
